...
`npm test` runs Vitest unit tests first, then the Playwright accessibility, component, flow, and visual suites. Unit tests are colocated under `src/**/__tests__/`; `npm run test:unit:coverage` writes coverage reports to `.logs/coverage/unit/`.
