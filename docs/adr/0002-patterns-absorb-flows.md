...
Patterns is a new top-level docs section (Get-started / Styles / Components / Patterns / Reference). Flows — interactive component journeys with human-testable scripts and Playwright coverage, established in [`/reference/conventions/flows-as-tests/`](../src/pages/reference/conventions/flows-as-tests.astro) — predate Patterns and previously lived at `/flows/`. The decision is to redefine Flows as a structural element within Pattern pages: a large Pattern (Check answers) embeds its Flow as the interactive demo and test surface; a small Pattern (Ask for an email) may have no Flow, only a static demo.
